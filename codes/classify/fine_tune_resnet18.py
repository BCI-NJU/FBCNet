...
# dataloader
train_dataloader = DataLoader(trainData, batch_size= batchsize)
val_dataloader = DataLoader(valData, batch_size= batchsize)
...
model.fc = nn.Linear(num_features, num_of_class) # binary classification (num_of_class == 2)
...
for epoch in range(num_epochs):
    ...
    epoch_acc = running_corrects / len(trainData) * 100.
    ...
    with torch.no_grad():
        ...
        epoch_loss = running_loss / len(valData)
        epoch_acc = running_corrects / len(valData) * 100.
        print('[Validation #{}] Loss: {:.4f} Acc: {:.4f}% Time: {:.4f}s'.format(epoch, epoch_loss, epoch_acc, time.time() - start_time))


# 模型加载
save_path = 'eeg_classification_transfer_learning_with_ResNet18.pth'
