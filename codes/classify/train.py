...
def spiltDataSet(trainDataToUse, testDataToUse, validationSet, data):
    ...
    # print(f"Length of valid data: {len(valData)};")
    # print(f"Length of test data: {len(testData)}.")
    # print(f"data shape: {testData[0]['data'].shape}")
    ...
    # 将测试集存成.npy文件
#     if not os.path.exists('TestDataOvR.npy'):
#         # 将 PyTorch 张量转换为 NumPy 数组
#         numpy_data_list = [{'data': item['data'].numpy(), 'label': item['label']} for item in testData]
#         # 保存 NumPy 数组
#         np.save('TestDataOvR.npy', numpy_data_list)

#     loaded_data_list = np.load('TestDataOvR.npy', allow_pickle=True)
